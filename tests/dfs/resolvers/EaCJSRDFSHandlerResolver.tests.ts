...
Deno.test(
  ...
  async (t) => {
    ...
    await t.step(
      "resolver passes FileRoot and filters paths correctly",
      async () => {
        ...
        const handler = await withRetry(async () => {
          return await EaCJSRDFSHandlerResolver.Resolve(
            ioc,
            "test-jsr",
            jsrDfs,
          );
        ...
        const paths = await withRetry(async () =>
          await handler!.LoadAllPaths()
        );
    ...
    await t.step(
      "resolver without FileRoot returns all package paths",
      async () => {
        const jsrDfs: EaCJSRDistributedFileSystemDetails = {
          Type: "JSR",
          Package: "@std/assert",
...
Deno.test(
  ...
  async (t) => {
    ...
    await t.step("throws for invalid DFS type", async () => {
      ...
      let error: Error | undefined;
      try {
        await EaCJSRDFSHandlerResolver.Resolve(
          ioc,
          "test-local",
          localDfs,
        );
