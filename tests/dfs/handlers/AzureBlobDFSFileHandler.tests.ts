...
Deno.test("AzureBlobDFSFileHandler Tests", async (t) => {
  ...
  await t.step("RemoveFile should successfully delete a file", async () => {
  ...
  await t.step("WriteFile should successfully upload a file", async () => {
    const testUploadFile = "test_upload.txt";

    const uploadStream = new ReadableStream<Uint8Array>({
      start(controller) {
        controller.enqueue(new TextEncoder().encode("Sample Uploaded Data"));
        controller.close();
      },
    });

    await handler.WriteFile(testUploadFile, "revision", uploadStream);

    const fileInfo = await handler.GetFileInfo(testUploadFile, "revision");
    assertEquals(
      fileInfo !== undefined,
      true,
